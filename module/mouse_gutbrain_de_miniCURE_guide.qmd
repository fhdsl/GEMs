...
To our eyes, not much has changed in the format, but it's  very different to R. Now we're going to tell R to name the rows based on the "GeneID" column, and then delete the current "GeneID" column.
...
heatmap(gutbrain_motor_controls)
```
...
```{r}
heatmap(gutbrain_motor_controls,  Colv = NA, Rowv = NA)
```
...
gutbrain_metadata <- read_csv("https://genomicseducation.org/data/mouse_gutbrain_metadata.csv")

#filter out non-controls and order columns by tissue type
