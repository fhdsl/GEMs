...
Let’s read in the data. We’ve hosted a snapshot on genomicseducation.org, so let’s read it in from that URL. Run the following code in your console:
...
1.  Run `count(phys_data, test, studentID)` to see student and test combinations. How many observations does student `S2` have for the Final? Take a screenshot showing this number.
