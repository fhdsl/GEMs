...
Electrodermal activity (EDA) is used in many contexts to measure human emotions, including in polygraph tests [@Synnott2015]. Scientists have found, even in tightly controlled experiments, that EDA is extremely variable [@amin2022wearable]. Some individuals have spikes of EDA, while others do not [@Synnott2015]. EDA can also be affected by environmental factors, such as the testing area's temperature and humidity, and the examiner's pre-existing beliefs [@macneill2016temperature; @elaad1994effects]. Do you think it's acceptable to use EDA to look at stress during exams? What about in a criminal justice setting? 
:::
