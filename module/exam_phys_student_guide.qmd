...
```{r, fig.align='center', echo = FALSE, fig.alt= "TO ADD", out.width = '100%'}
...
Type the following into your console and press return to run the code. You can use the **clipboard button** on the top right of the code block to copy everything. You will see a lot of text when the code is running, but usually that's a good thing!

```{r message=FALSE, eval=FALSE}
install.packages("tidyverse")
```

```{r, fig.align='center', echo = FALSE, fig.alt= "Paste the code above into the Posit Cloud console and click enter to run the code.", out.width = '100%'}
...
```{r, fig.align='center', echo = FALSE, fig.alt= "If tidyverse installs successfully, you'll see several 'DONE' messages and 'The downloaded source packages are in...'.", out.width = '100%'}
...
```{r, fig.align='center', echo = FALSE, fig.alt= "Successfully loading tidyverse with the library command shows that several other packages are attached.", out.width = '100%'}
...
```{r, fig.align='center', echo = FALSE, fig.alt= "Running the code above stores a data object in the environment called phys_data; some data is also printed in the console showing timestamp, eda, hr, test, and studentID columns.", out.width = '100%'}
...
```{r, fig.align='center', echo = FALSE, fig.alt= "Using the View command opens the data in a tabular browser, much like Excel or Google Sheets. Data cannot be edited in the Viewer.", out.width = '100%'}
...
```{r, fig.align='center', echo = FALSE, fig.alt= "Column names are surrounded by a box in the Viewer.", out.width = '100%'}
...
1.  Run `count(phys_data, test, studentID)` to see student and test combinations. How many observations does student `S1` have for Midterm 1? Take a screenshot showing this number.
...
```{r, fig.align='center', echo = FALSE, fig.alt= "Clicking on the column name sorts the timestamp column to reveal the earliest value.", out.width = '100%'}
...
```{r, fig.align='center', echo = FALSE, fig.alt= "Clicking the column name again sorts it in the other direction, revealing the latest timepoint.", out.width = '100%'}
...
```{r, fig.align='center', echo = FALSE, fig.alt= "Netflix stock price and the first name Stevie are strongly correlated, but there is no causation. Many correlation relationships are like this.", out.width = '100%'}
...
In this part, we'll compare two students.
...
3.  (Optional) To confirm your results, calculate the mean `eda` for each test and student. Return to the code from Part 4 to calculate the means, remembering to replace `hr` with `eda`. You already ran student `S9`, so you can just run `S8`. Do the results back up your conclusions? Take a screenshot of your results table in the console.
...
Let's looks at an additional source of data: the students' grades.
