...
::: callout-tip
## Electrodermal Activity and Criminal Justice

Electrodermal activity (EDA) is used in many contexts to measure human emotions, including in polygraph tests [@Synnott2015]. Scientists have found, even in tightly controlled experiments, that EDA is extremely variable [@Suryanarayanan2005]. Some individuals have spikes of EDA, while others do not [@Synnott2015]. EDA can also be affected by environmental factors, such as the testing area's temperature and humidity, and the examiner's pre-existing beliefs [@macneill2016temperature; @elaad1994effects]. Do you think it's acceptable to use EDA to look at stress during exams? What about in a criminal justice setting? 
:::
...
If there is a lot of output when you run code, simply scroll up and down in the console to view it all.
:::
...
In the data Viewer (top left), click on the variable/column names to sort them. For example, If we click `timestamp`, we can see the earliest timestamp is `2018-10-13 14:00:59`. We can click it again to see the latest timestamp is `2018-12-05 19:49:00`.
