...
There are three columns that you should pay close attention to.
...
- **padj**: the p-value for the log2FoldChange, adjusted for the fact you are testing thousands of genes. This is a standard adjustment done in statistics when you run a statistical test multiple times.

The dataset also contains other columns, but you won't be working with them specifically.
...
- pvalue: the uncorrected p-value for the log2FoldChange. 
:::
...
In part 3, we'll examine the gene expression in the prefrontal cortex for specific genes that have been previously associated with ASD and compare the expression to the gene expression in the striatum.

### Look up gene expression in the prefrontal cortex

Let's take a look at the differential expression (log2FoldChange) of gene ENSMUSG00000079516, which is the reg3a gene we previously looked up on MGI.
...
Based on the negative log2FoldChange value for reg3a, we know that this gene has _decreased expression_ in the ASD-type mice compared to control-type mice. If you click on the black triangle on the right side of the table, you can scroll through all the columns. The padj value  for reg3a is 2.63 x 10^-23^, which suggests the calculated decrease in expression is statistically significant.

That means reg3a is being downregulated in the prefrontal cortex of ASD-type mice!
...
Then we'll run code that lets us look at the gene expression of just reg3a in the striatum.
...
Interesting! The log2FoldChange of reg3a is positive, which _might_ mean reg3a has increased expression in the striatum when comparing ASD_type mice to control-type mice. However, if we scroll through all the columns (by clicking the black arrow on the right side of the table), we can see the padj hasn't been calculated. Hmm. Why would that be? This sometimes happens for gene expression data when a gene fails to pass a filtering test. Essentially, the software was unable to calculate an adjusted p-value for this gene.

However, we do have a non-adjusted p-value for reg3a expression in the striatum. Since this non-adjusted p-value is quite large (0.44!), we can safely assume reg3a does _not_ have different expression in ASD-type mice compared to control-type mice in the striatum.
