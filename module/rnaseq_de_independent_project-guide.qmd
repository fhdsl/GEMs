...
Let's say you want to open the dataset that has all the gene expression data (with no comparisons) and call it  `readCounts`. The command would be:

```{r}
readCounts <- read_csv("https://genomicseducation.org/data/mouse_autism_counts.csv")
```


###   Exploring the dataframe

This lets you explore how the dataset is formatted and how big it is, as well as what the various columns are named.
...
### Exploring differential expression of genes
...
```{r}
readCounts <- read_csv("https://genomicseducation.org/data/mouse_autism_deseq_autismVcontrol.csv")
```
