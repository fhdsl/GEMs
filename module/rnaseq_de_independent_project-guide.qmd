...
Let's say you want to open the dataset that compares gene expression between ASD and control mice in both brain regions and call it `asd_vs_c`. The command would be:

```{r}
asd_vs_c <- read_csv("https://genomicseducation.org/data/mouse_autism_deseq_autismVcontrol.csv")
```
...
```{r, eval = F}
asd_vs_c_prefrontal <- read_csv("https://genomicseducation.org/data/mouse_autism_deseq_autismVcontrol_in_prefrontalcortex.csv")

asd_vs_c_striatum <- read_csv("https://genomicseducation.org/data/mouse_autism_deseq_autismVcontrol_in_striatum.csv")
```

Then we'll filter out the gene in which we're interested from each object. Let's take a look at gene ENSMUSG00000059921, which is the unc-5 netrin receptor C gene.
“ENSMUSG00000079516”

```{r}
unc_prefrontal <- filter(asd_vs_c_prefrontal, gene == "ENSMUSG00000059921")

unc_striatum <- filter(asd_vs_c_striatum, gene == "ENSMUSG00000059921")
```

Next, we'll combine our two filtered datasets to make one single dataset that only contains expression of the unc-5 netrin receptor C gene. We'll first add a column to each dataset so we know whether we're looking at expression data in prefrontal cortex or striatum. Then we'll combine the datasets with the `bind_rows`. This command essentially "glues" the two datasets together, one on top of the other.
