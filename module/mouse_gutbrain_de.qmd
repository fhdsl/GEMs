...
1) a companion and expansion to the C-MOOR RNA-seq miniCURE. This module adds an additional RNA-seq dataset that is suitable for an independent research project.

2) a standalone activity that fits into a single laboratory session.
...
**Duration:** miniCURE: Three lab periods; standalone activity: 1-2 hours ([help us improve this estimate!](https://forms.gle/Qd9KLiDsUFY4s1LV8))
...
[Instructor survey](https://forms.gle/Qd9KLiDsUFY4s1LV8)
...
You can use this module in several formats. Feel free to adapt to your needs! We have two versions, one to accompany the C-MOOR miniCURE and one that can be used as a standalone activity.

-   MiniCURE add-on R student activity:
...
Students will need RStudio, data, and the R packages: `tidyverse`, `ggrepel` (miniCURE only).
...
A Google Slides presentation is available for borrowing images [here](https://docs.google.com/presentation/d/1vikcyml6uw-_DaC1yFpS-UWoWNEGxFukMB_eCi42BqY/edit?usp=sharing).
