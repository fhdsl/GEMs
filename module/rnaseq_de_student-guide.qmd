...
```{r}
prefrontal <- read_csv("https://genomicseducation.org/data/mouse_autism_deseq_autismVcontrol_in_prefrontalcortex.csv")
```
...
Let's take a look at the data you loaded into R. Type the following code into the console (this will open a new window):
...
**gene**: the geneID (see below for more detail). **baseMean**: the average of the normalized count values for this gene, taken over all samples.
...
The log2FoldChange value gives us an idea of which genes show the greatest differential expression between the ASD mice and the control mice. We can arrange the table based on these values (going from smallest to largest) by copying the following code into your console:
